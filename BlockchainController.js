    ...
    getBlockByHeight() {
        this.app.get("/blocks/height/:height", async (req, res) => {
            if(req.params.height) {
                ...
                return res.status(404).send("Block Not Found please Review the Parameters");
            }
    ...
    requestOwnership() {
        this.app.post("/requestValidation", async (req, res) => {
            if(req.body.address) {
                ...
                return res.status(500).send("error, check the body Parameter");
            }
        });
    }
    ...
    getStarsByOwner() {
        this.app.get("/blocks/address/:address", async (req, res) => {
            if(req.params.address) {
                ...
                return res.status(500).send("Block Not Found, please review the parameters");
            }
    ...
    validateChain(){
       this.app.get("/validateChain", async(req, res) =>{
           ...
           if(errorLog.length!=0){
               return res.status(500).send("Chain is incorect");
           }else{
               return res.status(200).send("Everything is correct");
           }
       });
    }
